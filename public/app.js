...
const UI_HIDDEN_ASSETS = new Set(["SAHARA"]);
...
function signalValidity(signal) {
  const ttlMinutes = Number(signal?.validForMinutes || 60);
  const explicitEnd = new Date(signal?.validUntil || signal?.priceAction?.lifecycleValidUntil || 0).getTime();
  if (Number.isFinite(explicitEnd) && explicitEnd > 0) {
    const remaining = Math.max(0, Math.ceil((explicitEnd - Date.now()) / 60_000));
    return { minutes: remaining, text: remaining > 0 ? `${remaining} menit` : "kedaluwarsa" };
  }
  const started = new Date(signal?.firstSeenAt || signal?.priceAction?.lifecycleFirstSeenAt || signal?.updatedAt || 0).getTime();
  if (!Number.isFinite(started) || !started) return { minutes: ttlMinutes, text: `${ttlMinutes} menit` };
  const remaining = Math.max(0, Math.ceil(ttlMinutes - (Date.now() - started) / 60_000));
  return { minutes: remaining, text: remaining > 0 ? `${remaining} menit` : "kedaluwarsa" };
}

function signalTradeStatus(signal) {
  const validity = signalValidity(signal);
  if (validity.minutes <= 0) return { code: "EXPIRED", label: "EXPIRED", className: "expired", reason: "Masa berlaku sinyal sudah habis." };
  const code = String(signal?.tradeStatus || signal?.priceAction?.tradeStatus || "WATCHLIST").toUpperCase();
  const reasons = signal?.executionReasons || signal?.priceAction?.executionReasons || [];
  if (code === "TRADE_READY") {
    const adaptive = Boolean(signal?.adaptiveWideStopApproved || signal?.priceAction?.adaptiveWideStopApproved);
    const confluence = Number(signal?.confluenceScore || signal?.priceAction?.confluenceScore || 0);
    if (adaptive) return { code, label: "ADAPTIVE READY", className: "ready", reason: `SL struktural lebar disetujui karena konfluensi kuat ${confluence}/11.` };
    return { code, label: "TRADE READY", className: "ready", reason: "Semua filter entry terpenuhi." };
  }
  return { code: "WATCHLIST", label: "WATCHLIST", className: "watch", reason: reasons[0] || "Setup belum memenuhi seluruh filter entry." };
}

function positionAgeLabel(position) {
  const opened = new Date(position?.openedAt || 0).getTime();
  if (!Number.isFinite(opened) || !opened) return "-";
  const totalMinutes = Math.max(0, Math.floor((Date.now() - opened) / 60_000));
...
function coinSymbol(coin) {
  ...
  if (code.startsWith("XAU") || code === "GC=F") return "Au";
...
function coinClass(coin) {
  ...
  if (code.startsWith("XAU") || code === "GC=F") return "coin-gold";
  return "coin-default";
}
...
function positionNotionalValue(position) {
  const explicit = Number(position?.notional);
  if (Number.isFinite(explicit) && explicit > 0) return explicit;
  const qty = Number(position?.qty || 0);
  const entry = Number(position?.entry || 0);
  return Number.isFinite(qty) && Number.isFinite(entry) ? qty * entry : Number(position?.margin || 0);
}

function positionLeverageValue(position) {
  const explicit = Number(position?.leverage);
  if (Number.isFinite(explicit) && explicit > 0) return explicit;
  const margin = Number(position?.margin || 0);
  return margin > 0 ? positionNotionalValue(position) / margin : 1;
}
...
function performanceSummary() {
  ...
  const recentWins = recent.filter(item => Number(item.pnl || 0) >= 0).length;
  ...
  return {
    ...
    statusText: history.length
      ? `All-time ${winRate.toFixed(0)}% • Recent ${recentWinRate.toFixed(0)}%/${recent.length} • Avg ${avgPnlPct.toFixed(2)}%`
...
function drawCandleChart(canvas, rows, options = {}) {
  ...
  })).filter(item => [item.o, item.h, item.l, item.c].every(Number.isFinite) && item.c > 0);

  const isGoldChart = ["XAUUSD", "GC=F"].includes(String(options.signal?.coin || options.signal?.symbol || "").toUpperCase());
  if (isGoldChart && data.length) {
    const guarded = [];
    for (const candle of data) {
      const recent = guarded.slice(-12).map(item => item.c).sort((a, b) => a - b);
      const middle = Math.floor(recent.length / 2);
      const reference = recent.length
        ? (recent.length % 2 ? recent[middle] : (recent[middle - 1] + recent[middle]) / 2)
        : candle.c;
      const extreme = [candle.o, candle.h, candle.l, candle.c].some(value => Math.abs(value - reference) / reference > 0.08);
      if (!extreme || guarded.length < 5) guarded.push(candle);
    }
    data = guarded;
  }
  ...
  const lastClose = data.at(-1)?.c || (candleMin + candleMax) / 2;
  const visibleDistance = Math.max((candleMax - candleMin) * 4, Math.abs(lastClose) * (isGoldChart ? 0.12 : 0.35));
  levels = levels.filter(level => Math.abs(level.value - lastClose) <= visibleDistance);
...
function buildEquitySeries(range = "1D") {
  const defaultStart = Number(state?.accounting?.initialCapital || 1000);
...
function normalizeBackendStatus(rawStatus) {
  const raw = String(rawStatus || "AI ACTIVE").replaceAll("_", " ").trim();
  const upper = raw.toUpperCase();
  if (upper.includes("ABORT") || upper.includes("TIMEOUT") || upper.includes("RETRYING")) return "BACKEND RETRYING";
  if (upper.includes("READY PARTIAL")) return "BACKEND READY";
  if (upper.includes("DEGRADED") || upper.includes("PRICE DELAYED") || upper.includes("DATA DELAYED")) return "DATA DELAYED";
  if (upper.startsWith("ERROR:")) return "BACKEND RETRYING";
  return raw;
}
...
function renderAll() {
  ...
  const status = normalizeBackendStatus(state.status);
  ...
  const lockedMargin = (state.positions || []).reduce((sum, position) => sum + Number(position.margin || 0), 0);
  const openPnl = (state.positions || []).reduce((sum, position) => sum + Number(position.unrealized || 0), 0);
  const accounting = state.accounting || {
    cashBalance: Number(state.balance || 0),
    lockedMargin,
    openPnl,
    computedEquity: Number(state.balance || 0) + lockedMargin + openPnl
  };
  const accountingBox = $("#accountingBreakdown");
  if (accountingBox) accountingBox.innerHTML = `
    <div><span>Modal awal</span><b>${money(accounting.initialCapital ?? 1000)}</b></div>
    <div><span>Realized P/L</span><b class="${Number(accounting.realizedPnl ?? 0) >= 0 ? "green" : "red"}">${pnlMoney(accounting.realizedPnl)}</b></div>
    <div><span>Open P/L</span><b class="${Number(accounting.openPnl || 0) >= 0 ? "green" : "red"}">${pnlMoney(accounting.openPnl)}</b></div>
    <div><span>Computed equity</span><b>${money(accounting.expectedEquity ?? accounting.computedEquity)}</b></div>
    <div><span>Cash balance</span><b>${money(accounting.cashBalance)}</b></div>
    <div><span>Locked margin</span><b>${money(accounting.lockedMargin)}</b></div>`;
...
function renderHome() {
  const best = [...currentSignals].sort((a, b) => {
    const readyDiff = Number(signalTradeStatus(b).code === "TRADE_READY") - Number(signalTradeStatus(a).code === "TRADE_READY");
    return readyDiff || Number(b.score || 0) - Number(a.score || 0);
  })[0];
  if (best) {
    $("#bestSignal").textContent = `${best.coin} ${best.side}`;
    const validity = signalValidity(best);
    $("#primeSubtext").textContent = `Analisis 1H • Target ${best.timeframe || "4-6 jam"} • valid ${validity.text}`;
    ...
    const primeStatus = signalTradeStatus(best);
    $("#primeInsight").innerHTML = `<span class="insight-icon">✦</span><p><b>${esc(primeStatus.label)}</b> • ${esc(primeStatus.code === "TRADE_READY" ? (best.why || best.technicalAnalysis || [primeStatus.reason])[0] : primeStatus.reason)}</p>`;
  ...
  homePositions.innerHTML = positions.length ? positions.map(position => `
    <div class="stack-row">
      <div class="row-title">
        ...
        <div><strong>${esc(position.coin)}</strong> ${badge(position.side)}<span class="row-sub">Entry ${money(position.entry)} • Margin ${money(position.margin)} • Notional ${money(positionNotionalValue(position))} • ${positionLeverageValue(position).toFixed(1)}x paper</span></div>
      </div>
...
function renderSignals() {
  ...
  box.innerHTML = visibleSignals.slice(0, 12).map((signal, index) => {
    ...
    const tradeStatus = signalTradeStatus(signal);
    return `<article class="signal-card uniform" data-signal-index="${index}">
      <div class="signal-top">
        <div>
          <div class="coin-title">${coinAvatar(signal.coin)}<span>${esc(signal.coin)}</span>${badge(signal.side)}<span class="trade-status ${tradeStatus.className}">${esc(tradeStatus.label)}</span></div>
          <p class="signal-meta-line">Analisis 1H • Target ${esc(signal.timeframe || "4-6 jam")} • Valid ${esc(signalValidity(signal).text)}</p>
        </div>
      ...
      <div class="signal-stat-grid compact-grid">
        ...
        <div class="signal-stat"><span>Confluence</span><b>${Number(signal.confluenceScore || signal.priceAction?.confluenceScore || 0)}/11</b></div>
        <div class="signal-stat"><span>Duration</span><b>${esc(signal.timeframe || "4-6 jam")}</b></div>
        <div class="signal-stat"><span>Risk Mode</span><b>${(signal.adaptiveWideStopApproved || signal.priceAction?.adaptiveWideStopApproved) ? "Adaptive" : "Standard"}</b></div>
        <div class="signal-stat"><span>Valid For</span><b>${esc(signalValidity(signal).text)}</b></div>
        <div class="signal-stat"><span>Structure</span><b>${esc(signal.priceAction?.pattern || "-")}</b></div>
      </div>
      ${levelStrip(signal)}
      <p class="signal-reason">${reason}</p>
      ${tradeStatus.code !== "TRADE_READY" ? `<p class="execution-note"><b>${esc(tradeStatus.label)}</b> • ${esc(tradeStatus.reason)}</p>` : `<p class="execution-note ready-note"><b>${esc(tradeStatus.label)}</b> • ${esc(tradeStatus.reason)}</p>`}
      ${(signal.confluenceReasons || signal.priceAction?.confluenceReasons || []).length ? `<p class="confluence-note"><b>Konfluensi:</b> ${esc((signal.confluenceReasons || signal.priceAction?.confluenceReasons || []).slice(0, 4).join(" • "))}</p>` : ""}
...
function renderPositions() {
  ...
  box.innerHTML = rows.map(position => {
    ...
    return `<div class="position-row">
      <div class="position-coin">${coinAvatar(position.coin)}<div><strong>${esc(position.coin)}</strong><small>${badge(position.side)} • aktif ${positionAgeLabel(position)} • margin ${money(position.margin)} • ${positionLeverageValue(position).toFixed(1)}x paper${position.riskMode === "ADAPTIVE_WIDE_STOP" ? " • adaptive SL" : ""}</small></div></div>
...
function openSignal(index) {
  ...
  $("#modalSub").textContent = `${signal.marketRegime || "Market setup"} • target ${signal.timeframe || "4-6 jam"} • valid ${signalValidity(signal).text}`;
  $("#modalBody").innerHTML = `
    <div class="detail-grid">
      ...
      <div class="detail-mini"><span>Confluence</span><b>${Number(signal.confluenceScore || signal.priceAction?.confluenceScore || 0)}/11</b></div>
      <div class="detail-mini"><span>Risk Mode</span><b>${(signal.adaptiveWideStopApproved || signal.priceAction?.adaptiveWideStopApproved) ? "Adaptive wide stop" : "Standard"}</b></div>
      <div class="detail-mini"><span>Trade Duration</span><b>${esc(signal.timeframe || "4-6 jam")}</b></div>
      <div class="detail-mini"><span>Signal Validity</span><b>${esc(signalValidity(signal).text)}</b></div>
    </div>
...
async function poll() {
  try {
    ...
    console.warn("State poll delayed:", error);
    $("#botStatus").textContent = "BACKEND RETRYING";
    $("#sidebarEngineStatus").textContent = "Using last saved data";
    $("#lastUpdatedText").textContent = "Koneksi tertunda, mencoba lagi";
  }
}
